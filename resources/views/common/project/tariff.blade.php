        ...
        <div class="community-tab__main-header">
            ...
            <div class="dropdown-red dropdown-red--left main-header__dropdown">
                ...
                <ul class="dropdown-red__list" data-dropdown-content>
                    <!--TODO как определить в какое сообщество добавлять  вариант тарифа? -->
                    @if($activeCommunity)
                        <li class="dropdown-red__item">
                            <a class="dropdown-red__link" href="{{ route('community.tariff.add', $activeCommunity) }}">
                                <span>{{ __('tariff.add_tariff') }}</span>
                            </a>
                        </li>
                        <!--TODO публикацию каких тарифов проводить ? для каких сообществ-->
                        <li class="dropdown-red__item">
                            <a class="dropdown-red__link"
                               href="{{ route('community.tariff.publication', $activeCommunity) }}">
                                <span>{{ __('tariff.tariffs_publication') }}</span>
                            </a>
                        </li>
                        <!--TODO как определять сообщество для каких настроек тарифа отображать форму-->
                        <li class="dropdown-red__item">
                            <a class="dropdown-red__link"
                               href="{{ route('community.tariff.settings', $activeCommunity) }}">
            ...
            >
                <option
                        value="{{ route('project.tariffs',array_filter([ 'project' => $project??'c', 'community'=> $community])) }}"
                        @if($isActive === true && $isPersonal !== true) selected @endif
                >
                    Активный
                </option>
                <option
                        value="{{ route('project.tariffs',array_filter([ 'project' => $project??'c', 'community'=> $community])) . '?active=false' }}"
                        @if($isActive === false) selected @endif
                >
                    Неактивный
                </option>
                <option
                        value="{{ route('project.tariffs',array_filter([ 'project' => $project??'c', 'community'=> $community])) . '?isPersonal=true' }}"
                        @if($isPersonal === true) selected @endif
                >
                    Персональный
                </option>
            </select>
                    ...
                    <li>
                        <div class="community-tariff-card community-tariff__item">
                            <div class="community-tariff-card__header">
                                ...
                                <a
                                        href="{{ route('community.tariff.edit', [$tariff->tariff->community_id, $tariff->id]) }}"
