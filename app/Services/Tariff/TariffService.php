...
class TariffService
{
    ...
    public function sendMessageAboutEndingTariff()
    {
        $collectionRecords = DB::table(DB::raw("telegram_users_tarif_variants as tutv"))
            ->join(DB::raw("tarif_variants as tv"), 'tv.id', '=', 'tutv.tarif_variants_id')
            ->join(DB::raw("tariffs as t"), 't.id', '=', 'tv.tariff_id')
    ...
    private function sendMessagesEmailBot($collectionRecords, $users, $telegram_users, $communities, $tariff_variants, $tarVars)
    {
        foreach ($collectionRecords as $record) {
            ...
            $tariff_variant_price = $tariff_variant->price;
            $tariff_variant_period = $tariff_variant->period;
            $community_name = $community->title;
            $days_left = $record->days_left;
            $date_tomorrow = Carbon::tomorrow()->translatedFormat('d M Y');

            $link = route('community.tariff.payment', ['hash' => PseudoCrypt::hash($community->id, 8)]);

            $textMessageView = view('mail.subscription_end', compact('user_name', 'tariff_variant_name', 'tariff_variant_period', 'community_name', 'days_left', 'recTarVars', 'link', 'tariff_variant_price'))->render();


            $textMessage = "Приветствуем $user_name! Напоминание об очередном платеже. Вы подписаны на сообщество $community_name. Стоимость вашей ежемесячной подписки $tariff_variant_price рублей. Списание суммы будет произведено автоматически $date_tomorrow года. Пожалуйста, проверьте, что на счету достаточно денежных средств и карта активна. Вы можете отключить регулярные платежи в любой момент - $link";
